...
export default function JourneyDashboard() {
  ...
  return (
        <div className="bg-white min-h-screen p-6 font-mono text-black border-8 border-black selection:bg-blue-600 selection:text-white">
  {/* Header */}
          <header className="border-b-8 border-black pb-4 mb-8 flex justify-between items-end">
            <div>
              <h1 className="text-6xl font-black uppercase tracking-tighter leading-none">GTM Journey</h1>
            <p className="text-lg mt-3 font-bold bg-blue-600 text-white inline-block px-3 py-1">CORE REVENUE FRAMEWORK</p>
    </div>
  ...
  {/* Nodes row */}
            ...
            {/* Horizontal axis line */}
                          <div className="absolute h-2 w-full bg-black top-1/2 transform -translate-y-1/2"></div>

  {journeyData.map((item, index) => (
                  <div key={index} className="relative flex flex-col items-center">
    {/* Number above node */}
                    <span className="mb-2 font-black text-base leading-none">{index + 1}</span>
                                   <motion.button
                                     whileHover={{ scale: 1.15, rotate: 5 }}
                    whileTap={{ scale: 0.9 }}
                  onClick={() => setActiveNode(item)}
                  className={`z-10 w-12 h-12 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex items-center justify-center ${
                                        activeNode?.stage === item.stage ? 'bg-blue-600 text-white' : 'bg-white'
                  }`}
                >
                  <div className={`w-3 h-3 rounded-full ${activeNode?.stage === item.stage ? 'bg-white' : 'bg-black'}`}></div>
                  </motion.button>
                  </div>
            ))}
              </div>
...
{/* Stage labels below nodes */}
          ...
          {journeyData.map((item, index) => (
                          <div key={index} className="flex-1 text-center text-[10px] font-black uppercase tracking-wider leading-tight px-0.5">
            {item.stage}
            </div>
                                       ))}
</div>
...
{/* Side Panel (Details) */}
        <div className="lg:col-span-1 border-4 border-black p-6 bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
        ...
        {activeNode ? (
              ...
              >
                ...
                <section>
                  ...
                  <p className="text-lg font-bold uppercase">{activeNode.alignment}</p>
                  </section>
                <section className="bg-black text-white p-3">
                  ...
                  <p className="text-base font-bold">! {activeNode.friction}</p>
                  </section>
                <section>
                                    <p className="text-xs font-black uppercase tracking-widest mb-1">Strategic Focus</p>
                  <p className="text-base font-bold underline decoration-blue-600 decoration-4">
                {activeNode.focus}
                  </p>
                  </section>
                <p className="text-sm leading-relaxed font-medium pt-3 border-t-2 border-black">
                {activeNode.details}
                  </p>
                  </motion.div>
            ) : (
                            <div className="h-full flex items-center justify-center text-center italic font-bold text-gray-400 uppercase text-sm min-h-48">
...
{/* Footer System Info */}
      <footer className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-black text-white p-5">
                  <h4 className="font-black text-lg uppercase mb-2">Current State Analysis</h4>
        ...
        <div className="border-4 border-black p-5">
                  <h4 className="font-black text-lg uppercase mb-2">Future State Vision</h4>
